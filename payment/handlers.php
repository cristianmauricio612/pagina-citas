...
function handle_autosubidas($pdo, $metadata)
{
  ...
  /*
    // 'au_start_day' => $fechaGenerada->format('Y-m-d'),
    // 'au_end_day' => $fechaGenerada->modify('+' . $autosubida['days'] . ' days')->format('Y-m-d'),
    'au_total' => $autosubida['times'] * $autosubida['days'],
  ]);
  */
  ...
  /**
    `au_active` TINYINT(1) DEFAULT 0,

    `au_start_day` DATE DEFAULT NULL,
    `au_end_day` DATE DEFAULT NULL,

    `au_days` int DEFAULT 0,
    `au_times` int DEFAULT 0,
    `au_interval` float DEFAULT 0,

    `au_total` int DEFAULT 0,
    `au_current` int DEFAULT 0,

    `au_start_hour` time DEFAULT 0,
    `au_end_hour` time DEFAULT 23,
   */
